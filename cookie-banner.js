...
function submitConsent(categories) {
    const agreedCookies = []
    if (categories === 'all' || categories === 'necessary') {
        agreedCookies.push(categories)
    } else if (categories === 'preference') {
    ...
    const consentId = 'AE1VSVI8T5'
    localStorage.setItem('PrivyConsent', consentId)
}
...
window.onload = () => {
    const consentVariable = localStorage.getItem('PrivyConsent')
    if(!consentVariable){
        const banner = createBanner(categorisedCookies, template)
